...
> el puerto 8086 (pasa al reiniciar el PC: la API vieja revive sin su
...
> ```
> NAMES                          STATUS                    PORTS
> proyecto_php3-api-facturas-1   Up 2 hours                0.0.0.0:8086->8086/tcp
> proyecto_php3-mariadb-1        Up 2 hours (healthy)      0.0.0.0:13328->3306/tcp
> ```
>
...
```powershell
git clone https://github.com/ccastro2050/proyecto_php3.git
cd proyecto_php3
...
| Qué | Dónde |
|---|---|
| **La pantalla** — empiece por aquí | **http://localhost:8084** |
| **API Facturas** — diagnóstico (dice qué motor está activo) | http://localhost:8086/ |
| Listar facturas (JSON) | http://localhost:8086/api/factura |
| **phpMyAdmin** (administrar MariaDB desde el navegador) | http://localhost:8103 |
| MariaDB (para SQLTools/DBeaver, opcional) | `localhost:13328` · `paradigmas`/`paradigmas123` |
| **PostgreSQL** (para DBeaver/SQLTools, opcional) | `localhost:15464` · `paradigmas`/`paradigmas123` |
...
**Empiece por el 8084**, que es como lo ve alguien que no programó esto. Use
el sistema un rato: cree una empresa, haga una factura de dos productos,
anúlela. Es exactamente lo que hacía la v2 — esta versión **no agrega ni una
funcionalidad**.

Y ahora, lo que sí es nuevo. **Mire el pie de la pantalla: dice MariaDB.**
Cambie el motor:
...
palabra. Vuelva a hacer todo lo de arriba y se siente igual.
...
> está aquí. **Son dos bases independientes**, y está declarado en el
> alcance: esta versión demuestra que el *programa* funciona contra las dos,
> no que las dos sean la misma.
...
Refresque <http://localhost:8084/facturas>: la pantalla sigue en pie, con su
...
> ℹ️ Este proyecto usa los puertos 8084, 8086, 8103, 13328 y 15464: si
> alguno ya está ocupado en su máquina, cámbielo en `docker-compose.yml` (el
> lado izquierdo del `"puerto:puerto"`).
...
> 🤖 ¿Va a trabajar con IA? Siga la **[Guía para construir la versión 3 con
> IA](docs/spec_kit/versiones/v3_segundo_motor/GUIA_IA3.md)** — cubre los dos
...
├── db/                          # LA MISMA base, en DOS dialectos (v3)
...
│
├── api_facturas/                # LA API — PHP puro, sin framework (puerto 8086)
...
│   ├── repositorios/            # Capa 3 — datos: interfaces + DOS implementaciones
...
│
├── front_php/                   # LA PANTALLA — PHP puro también (puerto 8084)
...
│                                # humo_los_dos_motores.py: corre lo anterior contra
│                                #   los dos motores — la prueba de la v3
├── docs/
...
v3  segundo motor (PostgreSQL) — la fábrica      ← USTED ESTÁ AQUÍ
                                                   (cerrada: tag v3)
...
| [2_spec.md](docs/spec_kit/versiones/v3_segundo_motor/2_spec.md) | QUÉ construir y los 13 criterios — **incluido el 13, que dice qué NO tocar** |
| [3_plan.md](docs/spec_kit/versiones/v3_segundo_motor/3_plan.md) | CÓMO: la fábrica, y **la tabla de las doce diferencias entre motores** |
...
| [5_data_model.md](docs/spec_kit/versiones/v3_segundo_motor/5_data_model.md) | La misma base, en dos dialectos: en qué se parecen y en qué no |
| [6_contracts.md](docs/spec_kit/versiones/v3_segundo_motor/6_contracts.md) | Los mismos de la v2 — **y que no cambiaran es el resultado** |
| [7_quickstart.md](docs/spec_kit/versiones/v3_segundo_motor/7_quickstart.md) | Smoke test para validar lo construido |
