...
```dockerfile
FROM php:8.3-cli
RUN apt-get update && apt-get install -y --no-install-recommends libpq-dev && docker-php-ex…
WORKDIR /app
COPY . .
EXPOSE 8086
CMD ["php", "-S", "0.0.0.0:8086", "index.php"]
```
...
| `EXPOSE 8086` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
| `CMD ["php", "-S", "0.0.0.0:8086", "index.ph…` | **El comando que se ejecuta al encender** el contenedor | Si ese proceso termina, el contenedor se apaga |
...
flowchart LR
    ...
    CLIENTE["Cliente de base de datos<br/>(DBeaver, SQLTools, mysql, psql)"]
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTPHP["SERVIDOR WEB (front)<br/>contenedor front-php<br/>hostname: front-php · escucha en 8084"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8086"]
    ...
    NAV -->|"localhost:8086"| APIFACTURAS
    NAV -->|"localhost:8084"| FRONTPHP
    NAV -->|"localhost:8103"| PHPMYADMIN
    APIFACTURAS -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    APIFACTURAS -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    FRONTPHP -->|"http://api-facturas:8086<br/>al controlador, por el NOMBRE"| APIFACTURAS
    PHPMYADMIN -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13328"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15464"| POSTGRES
```
...
**Sin compose**, para levantar este proyecto —que tiene **5 servicios**— hay
que escribir esto, en este orden, cada vez:

```powershell
# 1. Crear la red, para que los contenedores se encuentren por su nombre
docker network create proyecto_php3_default

# 2. mariadb
docker run -d --name mariadb --network proyecto_php3_default --restart unless-stopped `
  ...
  -v "${PWD}/db/mariadb/init_phpmyadmin.sql:/docker-entrypoint-initdb.d/init_phpmyadmin.sql:ro" `
  -p 13328:3306 mariadb:11

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. postgres
docker run -d --name postgres --network proyecto_php3_default --restart unless-stopped `
  ...
  -p 15464:5432 postgres:16-alpine

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. Construir la imagen de api-facturas y encenderla
docker build -t api-facturas ./api_facturas
docker run -d --name api-facturas --network proyecto_php3_default --restart unless-stopped `
  -e "MOTOR=${MOTOR:-mariadb}" `
  -e "DB_DSN_MARIADB=mysql:host=mariadb;port=3306;dbname=bdfacturas_mariadb_local" `
  -e "DB_DSN_POSTGRES=pgsql:host=postgres;port=5432;dbname=bdfacturas_postgres_local" `
  -e "DB_USUARIO=paradigmas" `
  -e "DB_CLAVE=paradigmas123" `
  -v "${PWD}/api_facturas:/app" `
  -p 8086:8086 api-facturas

# 7. Construir la imagen de front-php y encenderla
docker build -t front-php ./front_php
docker run -d --name front-php --network proyecto_php3_default --restart unless-stopped `
  -e "URL_API=http://api-facturas:8086" `
  -v "${PWD}/front_php:/app" `
  -p 8084:8084 front-php

# 8. phpmyadmin
docker run -d --name phpmyadmin --network proyecto_php3_default --restart unless-stopped `
  -e "PMA_HOST=mariadb" `
  -e "PMA_USER=paradigmas" `
  -e "PMA_PASSWORD=paradigmas123" `
  -p 8103:80 phpmyadmin:latest

```

**8 comandos**, con sus flags, en un orden que no se puede equivocar.
...
|---|---|---|
| **Su navegador** | `http://localhost:8086` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8086` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8086:8086`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8086`, el front
...
```powershell
docker network inspect proyecto_php3_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_php3` produce
la red `proyecto_php3_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_php3-front-php-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_php3_default has active endpoints (name:"proyecto_php3-front-php-1" …)
```
