...
## 6. Arreglos dentro de arreglos: el maestro y su detalle

Una factura no es una fila: es **una fila con una lista adentro**. El modelo
lo dice con todas las letras (`api_facturas/modelos/Factura.php`):

```php
private array $detalle;      // ← un array de líneas, dentro de la factura
```

Y en el JSON queda así:

```json
{
  "numero": 1,
  "detalle": [
    {"producto": "PR001", "cantidad": 2},
    {"producto": "PR002", "cantidad": 1}
  ]
}
```

La vista (`front_php/vistas/facturas_detalle.php`) lo recorre con un `foreach`
...
Dos cosas de ese renglón, que son las dos que hay que aprender:

1. **`$factura['detalle']` es un array de arrays.** Cada `$linea` es a su
   vez un diccionario, y se lee con `$linea['cantidad']`.
2. **El `?? []` no sobra.** Si la factura llegara sin `detalle` —porque la
   API cambió, porque hubo un error—, un `foreach` sobre `null` **revienta la
   página entera**. Con `?? []` recorre cero líneas y la pantalla sigue en
   pie. Es la diferencia entre una tabla vacía y una página en blanco.

---

## 7. Las variables superglobales: los arreglos que PHP llena solo
...
### 7.1 `$_SERVER` — el sobre de la petición
...
### 7.2 `$_GET` — lo que viene en la dirección
...
### 7.3 `$_POST` — lo que mandó el formulario
...
### 7.4 `$_SESSION` — lo que sobrevive a la redirección
...
### 7.5 `$GLOBALS` — el que se usa una vez y con motivo
...
### 7.6 Las que este proyecto NO usa, y por qué
...
### 7.7 La regla que vale para las cinco
...
## 8. Dónde aparecen los arreglos en la arquitectura de este proyecto
...
## 9. Diez errores clásicos (los que va a cometer)
...
## 10. Para practicar, con este código en la mano
