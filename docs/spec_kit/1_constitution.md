...
```
CAPA 1: FRONT (PHP, :8084)   — solo pinta HTML y llama la API; NUNCA toca la BD
CAPA 2: API  (PHP, :8086)    — api_facturas; solo JSON, nunca HTML
...
> **Desde la v3 este artículo dejó de ser una meta y pasó a ser estado.** Y
> se puede comprobar: cambie la variable `MOTOR`, reinicie la API, y el
> sistema entero sigue funcionando igual. El criterio 13 de la v3 lo verifica
> con un `diff`, no con una opinión.
...
| Cosa | Convención |
|---|---|
| Puertos públicos | front **8084** · api_facturas **8086** |
| Puertos de BD hacia el host | MariaDB **13328** · PostgreSQL **15464** (SQL Server llega en la v4) |
