...
| **v3** | **`proyecto_php3`** ← **USTED ESTÁ AQUÍ** | Lo mismo, **sin una funcionalidad nueva**, ahora también contra **PostgreSQL**, escogido por configuración | Nace la **fábrica** — abierto/cerrado en acción: cero cambios en controladores ni pantallas, y se comprueba con un `diff` |
...
## Qué cambia de la v2 a esta versión

| | v2 | v3 |
|---|---|---|
| Recursos | 6 | **los mismos 6** |
| Endpoints | 38 | **los mismos 38** |
| Pantallas | 15 | **las mismas 15** |
| Motores | MariaDB | **+ PostgreSQL, escogido por configuración** |
| Repositorios | 6 | **12** (seis por motor) |
| Controladores, servicios y vistas | — | **sin cambios** ← *el punto* |

**Ésta es la primera versión que no agrega ni una funcionalidad.** Y aun así
es la más importante de las cuatro, porque es la que **cobra las promesas**
de las dos anteriores.

La v1 y la v2 decían que el motor estaba encerrado en la capa de datos. Era
razonable, estaba escrito, y era imposible de verificar: **con un solo motor,
cualquier arquitectura parece independiente del motor.**

La v3 lo pone a prueba. Y el criterio de aceptación más importante no dice
qué construir: dice **qué no se puede haber tocado**, y se aprueba con un
`diff` contra la v2.
...
## Lo que la v4 va a poner a prueba

El mismo examen, una vez más y con un motor que se parece menos: **SQL
Server**. Si la v3 quedó bien hecha, agregarlo debería ser escribir seis
repositorios más y un traductor de errores más — y nada más.

Y hay una diferencia que la hace más dura: MariaDB y PostgreSQL comparten
buena parte del SQL estándar, tanto que las consultas de esta versión se
copiaron tal cual entre los dos. SQL Server se parece menos. Si algo del
dialecto se había colado sin que se notara, allá se va a ver.
