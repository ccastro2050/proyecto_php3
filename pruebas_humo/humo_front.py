# -*- coding: utf-8 -*-
"""Prueba de humo del FRONT de la v3 (PHP, puerto 8084).
...
Nada de la lista de arriba cambió. Lo que cambió es **cuántas veces se
corre**: este guion se ejecuta contra los DOS motores, y tiene que dar el
mismo resultado.

    python pruebas_humo/humo_front.py               contra el motor activo
    python pruebas_humo/humo_front.py --ambos       contra los dos, seguidos

Con `--ambos`, el guion reinicia la API cambiando la variable MOTOR y vuelve
a correrlo todo. Si un solo renglón sale distinto entre las dos corridas, la
promesa de la versión está rota — y la gracia es que el guion no sabe nada de
motores: solo llena formularios.

Y hay una comprobación nueva, la 10: que la pantalla diga contra qué motor
está corriendo, y que ese sea el que se pidió. Sin ella, las dos corridas
podrían estar dando contra la misma base sin que nadie lo notara.
...
# columna.
#
# FÍJESE EN QUE LOS NOMBRES DE LOS MOTORES YA NO ESTÁN EN ESTA LISTA, y no es
...
# contra qué motor está corriendo el sistema.
#
...
MOTORES_VISIBLES = ("MariaDB", "PostgreSQL")
for ruta, _ in PANTALLAS:
    c, t = ver(FRONT + ruta)
    texto = visible(t)
...
revisar("la API dice contra qué motor está corriendo",
        motor_real in ("mariadb", "postgres"), str(motor_real))
...
c, t = ver(FRONT + "/")
etiqueta = "PostgreSQL" if motor_real == "postgres" else "MariaDB"
